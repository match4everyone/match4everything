...
class AbstractParticipantInfo(models.Model):
    """
    This contains all the configurable proporties that can be used for matching participants to one another.
    ...
    def generate_fake(cls, participant, rs=None):
        """
        Generate a fake with random data, which is generated according to the type of its properties.
        """
        props = {}
    ...
    def get_absolute_url(self):
        return reverse('info-edit',
                       kwargs={'uuid': self.uuid,
                               'p_type': self.participant_type})
...
class ParticipantInfoA(AbstractParticipantInfo):
    participant_type = "A"
    participant = models.OneToOneField(Participant["A"], on_delete=models.CASCADE, primary_key=True,
                                       related_name='info')


class ParticipantInfoB(AbstractParticipantInfo):
    participant_type = "B"
    participant = models.OneToOneField(Participant["B"], on_delete=models.CASCADE, primary_key=True,
                                       related_name='info')
...
def add_participant_specific_info(name, config):
    ...
    for p in properties:
        info_cls.add_to_class(p.field_name,
                              p.get_field())

    info_cls.add_to_class('column_labels', labels)
    info_cls.add_to_class('random_generators', {p.field_name: p.get_random_value for p in properties})
