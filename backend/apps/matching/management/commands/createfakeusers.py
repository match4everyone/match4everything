...
from apps.matching.models import User, Participant, ParticipantInfo, ParticipantInfoLocation
...
class Command(BaseCommand):
    ...
    help = "Populates the database with fake users or deletes them." \
           "Every user has their email as their password."  # noqa: A003
    ...
    def handle(self, *args, **options):
        if (
                not options["delete"]
                and options["add_a"] is None
                and options["add_b"] is None
        ):
            self.print_help("", "")
            return None
    ...
    def add_fake(self, participant_type, n):
        ...
        self.stdout.write(self.style.SUCCESS("Created %s participants of type %s." % (n, participant_type)))
